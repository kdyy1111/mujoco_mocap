...
def key_callback_data(key, data):
    ...
    """
    if key == 265:  # Up arrow
        data.mocap_pos[1, 2] += 0.01
    elif key == 264:  # Down arrow
        data.mocap_pos[1, 2] -= 0.01
    elif key == 263:  # Left arrow
        data.mocap_pos[1, 0] -= 0.01
    elif key == 262:  # Right arrow
        data.mocap_pos[1, 0] += 0.01
    elif key == 320:  # Numpad 0
        data.mocap_pos[1, 1] += 0.01
    elif key == 330:  # Numpad .
        data.mocap_pos[1, 1] -= 0.01
    elif key == 260:  # Insert
        data.mocap_quat[1] = rotate_quaternion(data.mocap_quat[0], [1, 0, 0], 10)
    elif key == 261:  # Home
        data.mocap_quat[1] = rotate_quaternion(data.mocap_quat[0], [1, 0, 0], -10)
    elif key == 268:  # Home
        data.mocap_quat[1] = rotate_quaternion(data.mocap_quat[0], [0, 1, 0], 10)
    elif key == 269:  # End
        data.mocap_quat[1] = rotate_quaternion(data.mocap_quat[0], [0, 1, 0], -10)
    elif key == 266:  # Page Up
        data.mocap_quat[1] = rotate_quaternion(data.mocap_quat[0], [0, 0, 1], 10)
