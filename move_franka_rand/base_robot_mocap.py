...
def sample_position_around_center(center, r_min, r_max):
    # 위치 샘플링
    ...
    return np.array([x, y]), qz

def main():
    # Load the mujoco model basic.xml
    model = mujoco.MjModel.from_xml_path('base_scene.xml')
    data = mujoco.MjData(model)
    (x, y), qz = sample_position_around_center(center=(0, 0), r_min=1.1, r_max=1.3)
    ...
    data.qpos[:22] = [x, y, 0.1561732, qz[0], 5.81129e-05, -0.000731868, qz[3], 0.000589517, 0.00427167, 0.000753209, 
                -1.97983, -0.00550771, 2.02638, -2.18097, -2.38219e-06, 2.65126e-06, -0.0343745, 0.0850201, 0, 0, 0, 0]
    data.mocap_pos[0] = [x, y, 0.16]
    data.mocap_quat[0] = [qz[0], 0, 0, qz[3]]
    ...
    with mujoco.viewer.launch_passive(model, data, key_callback=key_callback) as viewer:
        while viewer.is_running():
            ...
            if now - last_update_time >= 3.0:
                # 예시: base를 random 위치로 이동 (자유 joint인 경우)
                # x, y = np.random.uniform(-1.0, 1.0, size=2)
                (x, y), qz = sample_position_around_center(center=(0, 0), r_min=1.1, r_max=1.3)
                data.qpos[:7] = [x, y, 0.16, qz[0], 0, 0, qz[3]] 
                data.qvel[:6] = 0
                print("x:",x,"y ",y)
                data.mocap_pos[0] = [x, y, 0.16]
                data.mocap_quat[0] = [qz[0], 0, 0, qz[3]]
